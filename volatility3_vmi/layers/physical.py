...
try:
    import libvmi
    from libvmi import Libvmi, VMIInitData
except ImportError:
    pass
...
vmi_instance = None
vmi_ref = 0
class VMILayer(interfaces.layers.DataLayerInterface):
    """a DataLayer backed by a libvmi session."""
    ...
    def __init__(self,
        ...
        global vmi_instance
        global vmi_ref
        if vmi_instance is not None:
            self.vmi = vmi_instance
            vmi_ref += 1
            return
    ...
    def read(self, offset: int, length: int, pad: bool = False) -> bytes:
        """Reads from the VMI interface."""
        buffer, bytes_read = self.vmi.read_pa(offset, length, True)
        return buffer

    def write(self, offset: int, data: bytes) -> None:
        """Writes to the VMI interface."""
        bytes_written = self.vmi.write_pa(offset, data)
        return bytes_written == len(data)

    def destroy(self) -> None:
        """Closes the vmi session."""
        if not hasattr(self, "vmi"):
            return

        global vmi_ref
        vmi_ref = max(vmi_ref - 1, 0)
