...
from volatility3.framework.layers import intel, intel_vmi, scanners
...
class LinuxIntelStacker(interfaces.automagic.StackerLayerInterface):
    ...
    def stack(
        ...
        if isinstance(layer, intel.Intel) or isinstance(layer, intel_vmi.IntelVMI):
            return None
        ...
        for _, banner in layer.scan(
            ...
            if isf_path:
                ...
                if context.config.get("plugins.stack.VMILayer.location", None):
                    vollog.info("Using IntelVMI layer")
                    layer_class: Type = intel_vmi.IntelVMI
                    ...
                    new_layer_name = context.layers.free_layer_name("IntelVMILayer")
                else:
