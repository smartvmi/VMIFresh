    ...
    addr_t *pgd)
{
    ...
    if ( VMI_FAILURE == vmi_translate_kv2p(vmi, *pgd, pgd))
        return VMI_FAILURE;

    if (vmi->init_flags & VMI_INIT_EVENTS)
    {
        rc = vmi_translate_kv2p(vmi, ts_addr + mm_offset, &phys);
        if ( VMI_FAILURE == rc )
            return rc;
    }

    pid_cache_set(vmi, pid, *pgd, phys);
    return VMI_SUCCESS;
}
