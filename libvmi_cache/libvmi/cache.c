...
struct pid_cache_removal {
    addr_t phys;
    vmi_instance_t vmi;
    char remain;
};
typedef struct pid_cache_entry *pid_cache_entry_t;
typedef struct pid_cache_removal *pid_cache_removal_t;
...
static gboolean
remove_pid_entry(gpointer key, gpointer val, gpointer data)
{
    (void) key;

    pid_cache_entry_t c = val;
    pid_cache_removal_t e = data;

    return c->mm == e->phys;
}

static void
filter_pid_entry(gpointer key, gpointer val, gpointer data)
{
    (void) key;
    ...
    addr_t start = (e->phys & ~(((addr_t) e->vmi->page_size) - 1));
    addr_t end = start + e->vmi->page_size;

    e->remain |= c->mm >= start && c->mm <= end;
}

char pid_event(vmi_instance_t vmi, vmi_event_t* event)
{
    /**
     * the memory cache may have removed the write events on this page,
     * however, if we are monitoring any task structs on the page,
     * we need to reenable them here.
     */
    struct pid_cache_removal r = { (event->mem_event.gfn << vmi->page_shift) + event->mem_event.offset, vmi, 0 };
    g_hash_table_foreach_remove(vmi->pid_cache, remove_pid_entry, &r);
    g_hash_table_foreach(vmi->pid_cache, filter_pid_entry, &r);
    return !r.remain;
}
    ...
    addr_t mm)
{
    ...
    *key = pid;
    ...
    entry = pid_cache_entry_create(pid, dtb, mm);
    if ( !entry ) {
        goto cleanup;
    }
    ...
    /*
     * Technically, we would need to enable VMI events on the mm page like this:
     * vmi_set_mem_event(vmi, (mm & ~(((addr_t) vmi->page_size) - 1)) >> vmi->page_shift, VMI_MEMACCESS_W, 0);
     * However, during address translation it already ends up in the page cache which enables it anyway.
     */
...
struct v2p_cache_removal {
    addr_t pt;
    vmi_instance_t vmi;
    char remain;
};
typedef struct v2p_cache_entry *v2p_cache_entry_t;
typedef struct v2p_cache_removal *v2p_cache_removal_t;

static gboolean
remove_v2p_entry(gpointer key, gpointer val, gpointer data)
{
    (void) key;
    v2p_cache_entry_t e = val;
    v2p_cache_removal_t r = data;
...
static gboolean
remove_v2p_entries(gpointer key, gpointer val, gpointer data)
{
    (void) key;
    GHashTable* v = val;
    g_hash_table_foreach_remove(v, remove_v2p_entry, data);
    return g_hash_table_size(v) == 0;
}

static void
filter_v2p_entry(gpointer key, gpointer val, gpointer data)
{
    (void) key;
    v2p_cache_entry_t e = val;
    v2p_cache_removal_t r = data;

    addr_t pt = (r->pt & ~(((addr_t) r->vmi->page_size) - 1));
    addr_t pte = (e->pte & ~(((addr_t) r->vmi->page_size) - 1));
    addr_t pgd = (e->pgd & ~(((addr_t) r->vmi->page_size) - 1));
    addr_t pdpte = (e->pdpte & ~(((addr_t) r->vmi->page_size) - 1));
    addr_t pml4e = (e->pml4e & ~(((addr_t) r->vmi->page_size) - 1));
...
static void
filter_v2p_entries(gpointer key, gpointer val, gpointer data)
{
    (void) key;
    GHashTable* v = val;
    g_hash_table_foreach(v, filter_v2p_entry, data);
}

static void
v2p_cache_entry_free(gpointer data)
{
...
char v2p_event(vmi_instance_t vmi, vmi_event_t* event)
{
    /* NOTE: this does not support nested paging. */
    struct v2p_cache_removal r = { (event->mem_event.gfn << vmi->page_shift) + event->mem_event.offset, vmi, 0 };
    g_hash_table_foreach_remove(vmi->v2p_cache, remove_v2p_entries, &r);
    g_hash_table_foreach(vmi->v2p_cache, filter_v2p_entries, &r);
    return !r.remain;
}
    ...
    page_info_t *info)
{
    ...
    key_128_t key = key_128_build(info->pt, info->npt);
    if ( !key )
        return;

    GHashTable *v = g_hash_table_lookup(vmi->v2p_cache, key);
    gboolean new_process_space = FALSE;
    addr_t * _va = NULL, va, pa;
    v2p_cache_entry_t entry = NULL;

    if ( !v ) {
        new_process_space = TRUE;

        v = g_hash_table_new_full(g_int64_hash, g_int64_equal, g_free, v2p_cache_entry_free);
        if ( !v )
            goto cleanup;
    ...
    entry = (v2p_cache_entry_t) g_malloc(sizeof(struct v2p_cache_entry));
    if ( !entry )
        goto cleanup2;

    entry->pa = pa;
    entry->pte = info->x86_ia32e.pte_location;
    entry->pgd = info->x86_ia32e.pgd_location;
    entry->pdpte = info->x86_ia32e.pdpte_location;
    entry->pml4e = info->x86_ia32e.pml4e_location;

    (void) g_hash_table_insert_compat(v, _va, entry);
